...
def logger(string, file_=open('log_ticket.txt', 'a'), lock=_thread.allocate_lock()):
    with lock:
        file_.write(string + '\n\n')
...
def threaded(c, addr):
    ...
    logger(
        f"### Counter {str(addr[0])} disconnect at: " + current_close_time + " ###")
...
def Main():
    ...
    while True:
        ...
        logger(
            f"Counter {str(addr[0])} connected at:   " + current_time + "UTC")
